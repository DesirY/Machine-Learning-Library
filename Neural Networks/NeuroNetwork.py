...
class NNetwork:
    def __init__(self, architecture, gama_0, d, T, training_set, threshold):
        ...
        self.B = [np.random.rand(l, 1) for l in architecture[1:]]   # the bias term, the output layer does not have bias term
        # print(self.B)
        for B in self.B:
            B = B.astype(np.float128)
        self.W = [np.random.rand(l, next_l) for l, next_l in zip(architecture[:-1], architecture[1:])]  # normal distribution
        for W in self.W:
